      ...
      while (true) {
        ...
        for (const line of lines) {
          try {
            ...
            if (data.text) assistantResponseText += data.text;
            if (data.sources) sourcesReceived = data.sources;
            if (data.error) {
            ...
            setMessages((prev) =>
              prev.map((msg) => 
                msg.id === assistantMessageId ? {...msg, text: assistantResponseText + (sourcesReceived.length > 0 ? `\n\n**Sources:**\n${sourcesReceived.map(s => `- [${s.filename}](${s.url})`).join('\n')}` : '')} : msg
              )
            );
          } catch (parseError) { console.error('Error parsing stream chunk:', parseError, 'Chunk:', line); }
        }
      }
      ...
      <div className="relative flex flex-col flex-1 max-w-5xl w-full mx-auto my-2 rounded-3xl shadow-2xl overflow-hidden bg-white">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-gray-50">
          {messages.map((message) => (
            message.text !== '...' && ( // Don't render the initial placeholder
              <div key={message.id} className={`flex ${message.sender ==='user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[75%] p-3 rounded-xl shadow-md ${message.sender === 'user' ? 'bg-blue-500 text-white rounded-br-none' : 'bg-white text-gray-800 rounded-bl-none border border-gray-200'}`}>
                <p className="text-sm leadingrelaxed" dangerouslySetInnerHTML={renderMarkdown(message.text)}></p>
                <span className={`text-xs mt-1 block ${message.sender === 'user' ? 'text-blue-200' : 'text-gray-500'} text-right`}>
                  {message.timestamp}
                </span>
                </div>
              </div>
        ...
        <div className="p-4 bg-white border-t border-gray-100 shadow-inner">
          <div className="flex items-center space-x-3">
            ...
            <button
              onClick={sendMessage}
              className={`p-3 rounded-full bg-blue-600 text-white flex items-center justify-center shadow-lg transition-all duration-200 ${isLoading ? 'opacity-50 cursor-not-allowed' : 'hover:bg-blue-700 active:scale-95'}`}
              disabled={isLoading}
            >
