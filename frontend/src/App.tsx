...
// For deployment, this might be dynamically set (e.g., from an environment variable or a config file).
...
export default function App() {
  ...
  const sendMessage = async () => {
    ...
    // Limit history to a reasonable number of turns to prevent excessively long requests
    const MAX_HISTORY_TURNS = 5; // Keep last 5 user and 5 assistant messages
    let historyForRequest = [...conversationHistory, { role: 'user', content: newUserMessage.text }];
    if (historyForRequest.length > MAX_HISTORY_TURNS * 2) {
      historyForRequest = historyForRequest.slice(-MAX_HISTORY_TURNS * 2); // Keep only the most recent turns
    }
    setConversationHistory(historyForRequest); // Update local history state
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/chat`, {
        ...
        body: JSON.stringify({
          ...
          history: historyForRequest, // Send the prepared conversation history
        }),
      });
